...
class CuraApplication(QtApplication):
    ...
    def readLocalFile(self, file: QUrl, project_mode: Optional[str] = None):
        ...
        #if extension == '.ss3d':
        #    extension = 'stl'
        #    f = '/home/akshay/Beer_Cap_Gun_Pusher_Block_v8.stl'
        ...
        #    print("MIN")
        #    print(filename)
        print("HIHIHIHIHIHIHIHIHIHIHIHIHIHIHIH")
        print(self._non_sliceable_extensions)
        print(self._currently_loading_files)
        ...
        if 'ss3d' in f: 
            ...
            input_file = f.replace('.ss3d','.stl')
            input_file = '/home/akshay/STLS/' + os.path.basename(input_file)
            print(input_file)
            with open(input_file,'wb') as file:
                file.write(decrypted)
            job = ReadMeshJob(input_file)
            self._currently_loading_files.append(input_file)
        else:
            job = ReadMeshJob(f)
            self._currently_loading_files.append(f)
...
#            os.remove(input_file)
    ...
    def checkIsValidProjectFile(self, file_url):
        ...
        print(file_path)
        if 'ss3d' in file_path:
            print("Checking File Validity")
            try:
                workspace_reader = self.getWorkspaceFileHandler().getReaderForFile(file_path)
                print(workspace_reader)
